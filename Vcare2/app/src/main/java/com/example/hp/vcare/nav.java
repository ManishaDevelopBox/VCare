...
public class nav extends AppCompatActivity
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.home) {
            ...
            FragmentTransaction ft=getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.frame,u1,"User Signin");
            ft.commit();
        } else if (id == R.id.help) {
            checkup c1=new checkup();
            FragmentTransaction ft=getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.frame,c1,"patient checkup");
            ft.commit();
        } else if (id == R.id.setting) {
            updatedetails u1=new updatedetails();
            FragmentTransaction ft=getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.frame,u1,"Update details");
            ft.commit();
        } else if (id == R.id.aus) {
            aboutus a1=new aboutus();
            FragmentTransaction ft=getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.frame,a1,"About us");
            ft.commit();
        } else if (id == R.id.cus) {
            contactus c1=new contactus();
            FragmentTransaction ft=getSupportFragmentManager().beginTransaction();
            ft.replace(R.id.frame,c1,"Contact us");
            ft.commit();
        }
        DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout);
        drawer.closeDrawer(GravityCompat.START);
